...
model.updateSeatList=function(list){
	var template='<tr>'+'<td class="seat-list-id">[seats.id]座</td>'+'<td class="seat-list-name">[seats.name]</td>'+'<td class="seat-list-time">[seats.time]</td>'+'<td class="seat-list-delete" onclick="rec(this)"><a class="am-close am-close-alt am-close-spin">&times;</a></td>'+'</tr>';
	this.seatList = list;
	...
	for(var i=0;i<this.seatList.length;i++){
		...
		var tr=template.replace('[seats.id]',seats.id).replace('_','排').replace('[seats.name]',getCookie('userName')).replace('[seats.time]',seatTime);
		//在DOM对象上绑定数据index
		tr = $(tr).data('index',i);
		tbody.append(tr);
	}
